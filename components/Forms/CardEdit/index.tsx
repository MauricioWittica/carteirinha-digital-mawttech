...
export const CardEditForm = () => {
    ...
    function handlesForm() {
            ...
            let data = {
                user_id: user?.id,
                birth_date: birthDateRef.current.value !== undefined ? birthDateRef.current.value : card.birth_date,
                filiation_date: filiationDateRef.current.value !== undefined ? filiationDateRef.current.value : card.filiation_date,
                zone: zoneRef.current.value !== undefined ? zoneRef.current.value : card.zone,
                section: sectionRef.current.value !== undefined ? sectionRef.current.value : card.section,
                city: cityRef.current.value !== undefined ? cityRef.current.value : card.city,
                state: stateRef.current.value !== undefined ? stateRef.current.value : card.state,
                documents: {
                    rg: rgRef.current.value !== undefined ? rgRef.current.value : card.documents.rg,
                    cpf: cpfRef.current.value !== undefined ? cpfRef.current.value : card.documents.cpf,
                    titulo: tituloRef.current.value !== undefined ? tituloRef.current.value : card.documents.titulo,
                }
    ...
    return (
        <View>
            ...
            <InputMask
                ...
                mask="date"
                value={filiationDate}
                placeholder={(card.filiation_date != undefined ? card.filiation_date : '00/00/0000')}
            ...
            <InputMask
                ...
                value={rg}
                placeholder={(card.documents.rg != undefined ? card.documents.rg : '00.000.000-0')}
                maxLength={12}
                inputRef={rgRef}
                keyboardType="numeric"
            ...
            <InputMask
                ...
                value={cpf}
                placeholder={(card.documents.cpf != undefined ? card.documents.cpf : '000.000.000-00')}
                maxLength={14}
                inputRef={cpfRef}
            ...
            <InputMask
                ...
                placeholder={(card.documents.titulo != undefined ? card.documents.titulo : '0000 0000 0000')}
                maxLength={14}
            ...
            <Text> </Text>
            <Button ref={buttonRef} title="Salvar Atualizações" onPress={() => { handlesForm() }} />
